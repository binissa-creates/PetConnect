  ...
  try {
    ...
    try {
      ...
      await db.query(
        ...
        [name, email, phone, barangay || null, clinic_name || null, license_number || null, emailAlertsVal, smsAlertsVal, req.userId]
      );
    } catch (dbErr) {
      // If columns don't exist, fall back to core columns
      if (dbErr.code === 'ER_BAD_FIELD_ERROR' || dbErr.message.includes('Unknown column')) {
        console.warn('Extended columns missing, falling back to core profile update');
    ...
    const [rows] = await db.query('SELECT id, name, email, phone, role FROM users WHERE id = ?', [req.userId]);
